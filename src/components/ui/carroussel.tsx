...
const Slide = ({ slide, index, current, handleSlideClick }: SlideProps) => {
  ...
  return (
    <div className="[perspective:1200px] [transform-style:preserve-3d]">
      <li
        ref={slideRef}
        className="flex flex-1 flex-col items-center justify-center relative text-center text-white opacity-100 transition-all duration-300 ease-in-out w-[90vmin] h-[50vmin] mx-[2vmin] z-10 "
...
export function Carousel({ slides }: CarouselProps) {
  // Commencer à la deuxième image (index 1) si elle existe, sinon à la première
  const [current, setCurrent] = useState(slides.length > 1 ? 1 : 0);

  // Animation automatique en boucle infinie
  useEffect(() => {
    if (slides.length <= 1) return; // Pas d'animation si une seule image
